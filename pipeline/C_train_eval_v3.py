...
OUTPUT_TAG     = "top2_per_sheet_v3_mod"    # v3 = v2 + treatment-modality features
...
# %% [Soft-vote ensemble across model families (v2 addition)]
# Build a soft-vote ensemble of all 4 families, then evaluate it head-to-head
# with the single-best on inner-CV OOF balanced accuracy. Whichever is better
# is used as the fold's final classifier. The ensemble uses every family's
# refit best_estimator_ and averages their predict_proba.
class SoftVoteEnsemble:
    def __init__(self, estimators):
        # estimators: dict name -> fitted estimator with predict_proba
...
# Inner-CV OOF probabilities for each family + the ensemble
